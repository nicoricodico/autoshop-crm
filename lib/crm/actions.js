  ...
  return {
    ...
    updateInvoice(id, patch) {
      run("Could not update invoice", async () => {
        ...
        if (patch.notes !== undefined) row.notes = patch.notes;
        if (patch.date !== undefined) row.invoice_date = new Date(patch.date).toISOString();
    ...
    deleteInvoice(id) {
      run("Could not delete invoice", async () => {
        // invoice_line_items and payments both reference invoices(id)
        // "on delete cascade", so deleting the invoice row removes them too.
